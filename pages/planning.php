...
if (isset($_POST['edit']) && !empty($_POST['id'])) {
    ...
    $reservAModifier = htmlspecialchars(trim($_POST['id']));
    $_SESSION['id_reservation'] = $reservAModifier;

    $sqlReservEdit = "SELECT * FROM reservation 
                  INNER JOIN salle ON salle.id = reservation.salle_id 
                  WHERE reservation.id = :id";

    // Préparer la requête
    $stmtReservEdit = $pdo->prepare($sqlReservEdit);

    // Exécuter la requête
    $stmtReservEdit->execute([
        'id' => $reservAModifier
    ]);

    // Récupérer le résultat sous forme de tableau associatif
    $resultatReservRecu = $stmtReservEdit->fetch(PDO::FETCH_ASSOC);
    

    $_SESSION['reservation'] = $resultatReservRecu;
