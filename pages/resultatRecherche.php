...
if (isset($_POST['submit'])) {
    if (!empty($_POST['choix_reservation'])) {
        ...
        if (!isset($_SESSION['reservation'])) {
            ...
            if ($resultat) {
            $success = "Votre reservation a bien ajouté en BDD ";
            $_SESSION=[];
            header("Refresh:2; url=index.php");


        } else {
            $error[] = "Erreur lors de la mettre à jour en BDD";
        }
        } else if (isset($_SESSION['reservation'])) {

            $id_reservation = $_SESSION['reservation']['id'];

            $sql = "UPDATE reservation 
        SET dateHeure_debut = :dateHeure_debut,
            dateHeure_fin   = :dateHeure_fin,
            nom             = :nom,
            salle_id        = :salle_id
        WHERE id = :id"; // l'ID de la réservation à modifier
            ...
            $resultat = $requete->execute([
                ...
                'nom'             => $nom,
                'salle_id'        => $id_salle,
                'id'              => $id_reservation // ici l'ID de la réservation
            ]);
        }

        if ($resultat) {
            $success = "Votre reservation a bien été mise à jour";
            $_SESSION=[];
